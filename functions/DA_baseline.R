...
AF_benefit_with_Risks <- function(x, varnames)
{
  ...
  #Adding up all crop-specific costs [â‚¬] to generate a value for the total cost associated with cultivating a specific crop
  
  #maize
  Treeless_maize_sowing_cost <- rep(0, n_years_c)#create vector with as many zeros as number of years in simulation
  Treeless_maize_sowing_cost[Maize_indices] <- vv(maize_seed_price_p, cv_maize_seed_price_c, length(Maize_indices)) * arable_area_treeless_c #fill vector with values at specific positions corresponding to the years maize is present in the crop rotation #cost of seed [â‚¬/ha]*area managed [ha]
  
  Treeless_maize_fertilizer_cost <- rep(0, n_years_c)
  Treeless_maize_fertilizer_cost[Maize_indices] <- vv(maize_fert_price_p, cv_maize_fert_price_c, length(Maize_indices)) * arable_area_treeless_c #cost of fertilizer [â‚¬/ha]*area managed [ha]
  
  Treeless_maize_pesticide_cost <- rep(0, n_years_c)
  Treeless_maize_pesticide_cost[Maize_indices] <- vv(maize_cides_price_p, cv_maize_cides_price_c, length(Maize_indices)) * arable_area_treeless_c #cost of pesticides [â‚¬/ha]*area managed [ha]
  
  Treeless_maize_machinery_cost <- rep(0, n_years_c)
  Treeless_maize_machinery_cost[Maize_indices] <- vv(maize_mach_price_p, cv_maize_mach_price_c, length(Maize_indices)) * arable_area_treeless_c #cost of machinery [â‚¬/ha]*area managed [ha]
  
  Treeless_maize_insurance_cost <- rep(0, n_years_c)
  Treeless_maize_insurance_cost[Maize_indices] <- vv(maize_insurance_p, cv_maize_insurance_c, length(Maize_indices)) * arable_area_treeless_c #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  Treeless_wheat_sowing_cost[Wheat_indices] <- vv(wheat_seed_price_p, cv_wheat_seed_price_c, length(Wheat_indices)) * arable_area_treeless_c #cost of seed [â‚¬/ha]*area managed [ha]
  
  Treeless_wheat_fertilizer_cost <- rep(0, n_years_c)
  Treeless_wheat_fertilizer_cost[Wheat_indices] <- vv(wheat_fert_price_p, cv_wheat_fert_price_c, length(Wheat_indices)) * arable_area_treeless_c #cost of fertilizer [â‚¬/ha]*area managed [ha]
  
  Treeless_wheat_pesticide_cost <- rep(0, n_years_c)
  Treeless_wheat_pesticide_cost[Wheat_indices] <- vv(wheat_cides_price_p, cv_wheat_cides_price_c, length(Wheat_indices)) * arable_area_treeless_c #cost of pesticides [â‚¬/ha]*area managed [ha]
  
  Treeless_wheat_machinery_cost <- rep(0, n_years_c)
  Treeless_wheat_machinery_cost[Wheat_indices] <- vv(wheat_mach_price_p, cv_wheat_mach_price_c, length(Wheat_indices)) * arable_area_treeless_c #cost of machinery [â‚¬/ha]*area managed [ha]
  
  Treeless_wheat_insurance_cost <- rep(0, n_years_c)
  Treeless_wheat_insurance_cost[Wheat_indices] <- vv(wheat_insurance_p, cv_wheat_insurance_c, length(Wheat_indices)) * arable_area_treeless_c #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  Treeless_barley_sowing_cost[Barley_indices] <- vv(barley_seed_price_p, cv_barley_seed_price_c, length(Barley_indices)) * arable_area_treeless_c #cost of seed [â‚¬/ha]*area managed [ha]
  
  Treeless_barley_fertilizer_cost <- rep(0, n_years_c)
  Treeless_barley_fertilizer_cost[Barley_indices] <- vv(barley_fert_price_p, cv_barley_fert_price_c, length(Barley_indices)) * arable_area_treeless_c #cost of fertilizer [â‚¬/ha]*area managed [ha]
  
  Treeless_barley_pesticide_cost <- rep(0, n_years_c)
  Treeless_barley_pesticide_cost[Barley_indices] <- vv(barley_cides_price_p, cv_barley_cides_price_c, length(Barley_indices)) * arable_area_treeless_c #cost of pesticides [â‚¬/ha]*area managed [ha]
  
  Treeless_barley_machinery_cost <- rep(0, n_years_c)
  Treeless_barley_machinery_cost[Barley_indices] <- vv(barley_mach_price_p, cv_barley_mach_price_c, length(Barley_indices)) * arable_area_treeless_c #cost of machinery [â‚¬/ha]*area managed [ha]
  
  Treeless_barley_insurance_cost <- rep(0, n_years_c)
  Treeless_barley_insurance_cost[Barley_indices] <- vv(barley_insurance_p, cv_barley_insurance_c, length(Barley_indices)) * arable_area_treeless_c #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  Treeless_rapeseed_sowing_cost[Rapeseed_indices] <- vv(rapeseed_seed_price_p, cv_rapeseed_seed_price_c, length(Rapeseed_indices)) * arable_area_treeless_c #cost of seed [â‚¬/ha]*area managed [ha]
  
  Treeless_rapeseed_fertilizer_cost <- rep(0, n_years_c)
  Treeless_rapeseed_fertilizer_cost[Rapeseed_indices] <- vv(rapeseed_fert_price_p, cv_rapeseed_fert_price_c, length(Rapeseed_indices)) * arable_area_treeless_c #cost of fertilizer [â‚¬/ha]*area managed [ha]
  
  Treeless_rapeseed_pesticide_cost <- rep(0, n_years_c)
  Treeless_rapeseed_pesticide_cost[Rapeseed_indices] <- vv(rapeseed_cides_price_p, cv_rapeseed_cides_price_c, length(Rapeseed_indices)) * arable_area_treeless_c #cost of pesticides [â‚¬/ha]*area managed [ha]
  
  Treeless_rapeseed_machinery_cost <- rep(0, n_years_c)
  Treeless_rapeseed_machinery_cost[Rapeseed_indices] <- vv(rapeseed_mach_price_p, cv_rapeseed_mach_price_c, length(Rapeseed_indices)) * arable_area_treeless_c #cost of machinery [â‚¬/ha]*area managed [ha]
  
  Treeless_rapeseed_insurance_cost <- rep(0, n_years_c)
  Treeless_rapeseed_insurance_cost[Rapeseed_indices] <- vv(rapeseed_insurance_p, cv_rapeseed_insurance_c, length(Rapeseed_indices)) * arable_area_treeless_c #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  AF_planning_cost <- rep(0, n_years_c) #Invoice of service provider (planners/consultants with fudning to support consultation), planning the AF system [â‚¬] + amount of time spent by the farmer to conceptualize the system
  AF_planning_cost[1] <- farmer_planning_time_p * labour_cost_p
  ...
  AF_planning_cost_subs <- rep(0, n_years_c)
  AF_planning_cost_subs[1] <- pmax(0, consult_cost_net) + farmer_planning_time_p * labour_cost_p
  
  AF_pruning_course <- rep(0, n_years_c) #Costs of the pruning training of one employee [â‚¬]
  AF_pruning_course[1] <- pruning_course_p
  
  #Field prep
  AF_gps_measuring <- rep(0, n_years_c) #First step of implementation: measuring tree strips using GPS[â‚¬]
  AF_gps_measuring[1] <- gps_field_measuring_p * labour_cost_p
  
  AF_dig_plant_holes <- rep(0, n_years_c) #Second step of implementation: digging/drilling holes for the trees [â‚¬]
  AF_dig_plant_holes[1] <- dig_planting_holes_p * labour_cost_p
  
  AF_tree_cost <- rep(0, n_years_c) #Cost per tree [â‚¬]
  AF_tree_cost[1] <- pmax(0,((appletree_price_p * num_trees_c) -(funding_onetime_per_tree_schemes_c * num_trees_c)))
  
  AF_plant_tree_cost <- rep(0, n_years_c) #Labour cost for planting one tree [â‚¬]
  ...
  AF_weed_protect_cost <- rep(0, n_years_c) #Material cost of weed suppressing fleece [â‚¬]
  AF_weed_protect_cost[1] <- weed_protection_p * num_trees_c
  
  AF_compost_cost <- rep(0, n_years_c) #Cost of compost used during planting [â‚¬]
  AF_compost_cost[1] <- compost_after_planting_p * compost_price_c * num_trees_c
  
  AF_irrigation_system_cost <- rep(0, n_years_c) #Material and labour cost of installing a drip irrigation system in the tree rows [â‚¬]
  ...
  AF_irrigation_after_planting_cost <- rep(0, n_years_c) #Cost for watering in newly planted trees [â‚¬]
  ...
  AF_investment_cost <- AF_planning_cost + AF_pruning_course
  ...
  AF_total_planting_cost_subs <- pmax(0, AF_planting_cost_after_fixed)
  
  AF_investment_cost_subs <-  AF_total_planting_cost_subs + AF_pruning_course + LEADER_application #Investment cost of AF system implementation
  
  AF_total_investment_cost_subs <- pmax(0, AF_investment_cost_subs - (LEADER_subsidy[1] + Onetime_external_support[1]))
  
  
  #Running costs
  ES3_application <- rep(0, n_years_c) #Time (regarded as labour cost) spent for application of Eco Scheme subsidy [â‚¬]
  ...
  AF_pruning <- rep(0, n_years_c)#Labour cost of pruning fruit trees [â‚¬]
  ...
  AF_root_pruning <- rep(0, n_years_c) #Labour cost of pruning roots of trees next to tree rows [â‚¬]
  AF_root_pruning[1:n_years_c] <- root_pruning_p * labour_cost_p
  
  AF_annual_irrigation <- rep(0, n_years_c) #Cost of annual irrigation of tree rows [â‚¬]
  ...
  AF_codling_moth_protect <- rep(0, n_years_c) #Cost of hanging up pheromone dispensers for codling moth control (includes material and labour cost) [â‚¬]
  AF_codling_moth_protect[1:4] <- 0
  AF_codling_moth_protect[4:n_years_c] <- codling_moth_protect_p * tree_row_area_c
  
  AF_mowing_treerow <- rep(0, n_years_c) #Labour cost of mowing the tree rows manually [â‚¬]
  ...
  AF_apple_harvest <- rep(0, n_years_c) #Labour cost of harvesting apples manually [â‚¬/kg]
  ...
  AF_total_treerow_management_cost <- AF_pruning + AF_root_pruning + AF_annual_irrigation_cost + AF_codling_moth_protect + AF_mowing_treerow + AF_apple_harvest + AF_processing_facility_running_cost
  
  #Management cost of arable component in AF system
  #Maize
  AF_maize_sowing_cost <- Treeless_maize_sowing_cost * AF_arable_area_perc #cost of seed [â‚¬/ha]*area managed [ha]
  AF_maize_fertilizer_cost <- Treeless_maize_fertilizer_cost*AF_arable_area_perc #cost of fertilizer [â‚¬/ha]*area managed [ha]
  AF_maize_pesticide_cost <- Treeless_maize_pesticide_cost*AF_arable_area_perc #cost of pesticides [â‚¬/ha]*area managed [ha]
  AF_maize_machinery_cost <- Treeless_maize_machinery_cost*AF_arable_area_perc #cost of machinery [â‚¬/ha]*area managed [ha]
  AF_maize_insurance_cost <- Treeless_maize_insurance_cost*AF_arable_area_perc #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  #Wheat
  AF_wheat_sowing_cost <- Treeless_wheat_sowing_cost*AF_arable_area_perc #cost of seed [â‚¬/ha]*area managed [ha]
  AF_wheat_fertilizer_cost <- Treeless_wheat_fertilizer_cost*AF_arable_area_perc #cost of fertilizer [â‚¬/ha]*area managed [ha]
  AF_wheat_pesticide_cost <- Treeless_wheat_pesticide_cost*AF_arable_area_perc #cost of pesticides [â‚¬/ha]*area managed [ha]
  AF_wheat_machinery_cost <- Treeless_wheat_machinery_cost*AF_arable_area_perc #cost of machinery [â‚¬/ha]*area managed [ha]
  AF_wheat_insurance_cost <- Treeless_wheat_insurance_cost*AF_arable_area_perc #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  #Barley
  AF_barley_sowing_cost <- Treeless_barley_sowing_cost*AF_arable_area_perc #cost of seed [â‚¬/ha]*area managed [ha]
  AF_barley_fertilizer_cost <- Treeless_barley_fertilizer_cost*AF_arable_area_perc #cost of fertilizer [â‚¬/ha]*area managed [ha]
  AF_barley_pesticide_cost <- Treeless_barley_pesticide_cost*AF_arable_area_perc #cost of pesticides [â‚¬/ha]*area managed [ha]
  AF_barley_machinery_cost <- Treeless_barley_machinery_cost*AF_arable_area_perc #cost of machinery [â‚¬/ha]*area managed [ha]
  AF_barley_insurance_cost <- Treeless_barley_insurance_cost*AF_arable_area_perc #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  #Rapeseed
  AF_rapeseed_sowing_cost <- Treeless_rapeseed_sowing_cost*AF_arable_area_perc #cost of seed [â‚¬/ha]*area managed [ha]
  AF_rapeseed_fertilizer_cost <- Treeless_rapeseed_fertilizer_cost*AF_arable_area_perc #cost of fertilizer [â‚¬/ha]*area managed [ha]
  AF_rapeseed_pesticide_cost <- Treeless_rapeseed_pesticide_cost*AF_arable_area_perc #cost of pesticides [â‚¬/ha]*area managed [ha]
  AF_rapeseed_machinery_cost <- Treeless_rapeseed_machinery_cost*AF_arable_area_perc #cost of machinery [â‚¬/ha]*area managed [ha]
  AF_rapeseed_insurance_cost <- Treeless_rapeseed_insurance_cost*AF_arable_area_perc #cost of insurance [â‚¬/ha]*area managed [ha]
  ...
  AF_machinery_cost <- AF_maize_machinery_cost + AF_wheat_machinery_cost + AF_barley_machinery_cost + AF_rapeseed_machinery_cost
  AF_labour_cost <- AF_maize_labour_cost + AF_wheat_labour_cost + AF_barley_labour_cost + AF_rapeseed_labour_cost
  
  
  AF_total_running_cost <- AF_total_treerow_management_cost + AF_total_arable_management_cost #Total running cost of AF system
  
  AF_total_cost <- AF_investment_cost + AF_total_running_cost #Total cost of AF system
  
  AF_total_benefit <- AF_apple_benefit + AF_maize_benefit + AF_wheat_benefit + AF_barley_benefit + AF_rapeseed_benefit 
  
  AF_bottom_line_benefit <- AF_total_benefit - AF_total_cost
 
  # With subsidies and incentives
  AF_total_treerow_management_cost_subs <- AF_total_treerow_management_cost + Digital_tool_subscription + ES3_application 
  
  AF_total_running_cost_subs <- AF_total_treerow_management_cost_subs + AF_total_arable_management_cost #Total running cost of AF system
  
  AF_total_cost_subs <- AF_total_investment_cost_subs + AF_total_running_cost_subs #Total cost of AF system
  ######AF_total_cost_subs <- AF_total_investment_cost + AF_total_running_cost #Total cost of AF system
  
  AF_total_benefit_subs <- AF_total_benefit + ES3_subsidy + AF_hedge_treerow_funding + AF_per_tree_funding + Annual_external_support 
  ...
  #AF System with selected subsidies and incentives
  AF_NPV_subs <- discount(AF_bottom_line_benefit_subs, discount_rate=discount_rate_p,
                     calculate_NPV = TRUE)#NVP of AF system
  AF_cash_flow_subs <- discount(AF_bottom_line_benefit_subs,discount_rate=discount_rate_p,
                           calculate_NPV = FALSE)# Cash flow of AF system
  AF_cum_cash_flow_subs <- cumsum(AF_cash_flow_subs) #Cumulative cash flow of AF system
  #Treeless System
  Treeless_NPV <- discount(Treeless_bottom_line_benefit, discount_rate=discount_rate_p,
                           calculate_NPV = TRUE)#NVP of treeless system
  Treeless_cash_flow <- discount(Treeless_bottom_line_benefit,discount_rate=discount_rate_p,
                                 calculate_NPV = FALSE)#Cash flow of AF system
  ...
  return(list(
    #Outputs: Treeless
    Treeless_NPV = Treeless_NPV,
    Treeless_cash_flow = Treeless_cash_flow,
    Treeless_cum_cash_flow = Treeless_cum_cash_flow,
    #Output: AF
    AF_NPV = AF_NPV,
    AF_cash_flow = AF_cash_flow,
    AF_cum_cash_flow = AF_cum_cash_flow,
    #Output: AF with subsidies
    AF_NPV_subs = AF_NPV_subs,
    AF_cash_flow_subs = AF_cash_flow_subs,
    AF_cum_cash_flow_subs = AF_cum_cash_flow_subs,
    #Output:
    NPV_decision = NPV_decision,
    CF_decision = CF_decision,
    CumCF_decision = CumCF_decision,
    #Outputs required for other functions
    ...
    consultation_cost = planning_consulting_p,
    machinery_cost = AF_machinery_cost,
    labour_cost =  AF_labour_cost,
    digital_tool_subscription = Digital_tool_subscription,
    Table_apple_yield = Table_apple_yield,
    B_qual_table_apple_yield = B_qual_table_apple_yield,
    Juice_apple_yield = Juice_apple_yield,
    AF_maize_yield = AF_maize_yield,
    AF_wheat_yield = AF_wheat_yield,
    AF_barley_yield = AF_barley_yield,
    AF_rapeseed_yield = AF_rapeseed_yield,
    table_apple_price_market = table_apple_price_market,
    B_qual_apple_price_market = B_qual_apple_price_market,
    Juice_apple_price_market = Juice_apple_price_market
